...
conditions = [
  ...
  Xsource['option'].isin(['not voting', 'absent', 'not member'])
]
...
for i, filtered_df in enumerate(new_dfs):
  ...
  fdf = filtered_df[filtered_df.index.isin(XrawTc.index)]
  XrawTc[~fdf['in_time_period']] = np.nan
...
mps = pd.read_csv(localpath + "data/mps.csv")

columns = ['voter_id', 'dim1', 'dim2', 'dim3', 'priezvisko', 'meno', 'kand. listina']
out = pd.DataFrame(columns=(columns+ ['datum']))

for i, projection in enumerate(projections):
  p = projection.iloc[:, 0:3]
  p.columns = ['dim1', 'dim2', 'dim3']
  p.reset_index(inplace=True)
  p = p.merge(mps.loc[:, ['mp_id', 'family_name', 'given_name', 'list']], left_on='voter_id', right_on='mp_id', how='left')
  p['datum'] = i
  del p['mp_id']
  p.columns = columns + ['datum']
